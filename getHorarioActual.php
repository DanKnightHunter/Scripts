    ...
    /*$query = "select idHorarioSeccion, horaInicio, horaFin, dia from alumno, profesor,
                materia, secciones_alumnos, seccion, horario_seccion WHERE alumno.valido=true 
                AND alumno.idAlumno = secciones_alumnos.idAlumno
                AND secciones_alumnos.idSeccion = seccion.idSeccion
                AND materia.idMateria = seccion.idMateria
                AND profesor.idProfesor = seccion.idProfesor
                AND horario_seccion.idSeccion = seccion.idSeccion
                AND alumno.valido=true"." AND alumno.idAlumno=".$id.
                " ORDER BY alumno.idAlumno;";
    $query_exec = mysqli_query($conexion, $query);
    //$salida = array("dato" => "a", "nombre" => "Andres");
    if(mysqli_num_rows($query_exec)){
